# ================================================================
#  A4 FORM PAGE — Section 2 Medical Recommendation for Assessment
#  Mental Health Act 1983 - Form A4 Regulation 4(1)(d)(ii)
#  Single practitioner (unlike A3 which is joint)
...
# ================================================================
# A4 CARD WIDGET - Fixed header, scrollable content
# ================================================================
class A4CardWidget(QFrame):
...
# TOOLBAR
# ================================================================
class A4Toolbar(QWidget):
    """Toolbar for the A4 Form Page."""
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("""
            A4Toolbar { background: rgba(200, 215, 220, 0.95); border-bottom: 1px solid rgba(0,0,0,0.12); }
...
# ================================================================
# MAIN A4 FORM PAGE - Card/Popup Layout
# ================================================================
class A4FormPage(QWidget):
    """Page for completing MHA Form A4 - Section 2 Single Medical Recommendation for Assessment."""
    ...
    def _setup_ui(self):
        ...
        title = QLabel("Form A4 — Section 2 Medical Recommendation (Single)")
    ...
    def _create_patient_card(self):
        ...
        card = A4CardWidget("Patient", "patient")
    ...
    def _create_practitioner_card(self):
        ...
        card = A4CardWidget("Practitioner", "practitioner")
        card.clicked.connect(self._on_card_clicked)
        self._hook_editor_focus(card.content)
    ...
    def _create_clinical_card(self):
        ...
        card = A4CardWidget("Clinical Reasons", "clinical")
        card.clicked.connect(self._on_card_clicked)
        self._hook_editor_focus(card.content)
    ...
    def _create_signature_card(self):
        ...
        card = A4CardWidget("Assessment & Signature", "signatures")
        card.clicked.connect(self._on_card_clicked)
        self._hook_editor_focus(card.content)
    ...
    def _create_clinical_popup(self):
        ...
        self.assessment_resistant_cb = QCheckBox("Assessment resistant")
        self.assessment_resistant_cb.setStyleSheet("font-size: 20px; color: #6b7280;")
        nature_opt_layout.addWidget(self.assessment_resistant_cb)
    ...
    def _create_signature_popup(self):
        ...
        sig_header = QLabel("Signature Date")
    ...
    def _update_signature_card(self):
        ...
        self.cards["signatures"].set_content_text(sig_date)
    ...
    def _connect_clinical_live_preview(self):
        ...
        # Nature sub-options
        for cb in [self.relapsing_cb, self.assessment_resistant_cb, self.chronic_cb]:
            cb.toggled.connect(self._update_clinical_preview)
    ...
    def _on_nature_toggled(self, checked):
        ...
        if not checked:
            ...
            self.assessment_resistant_cb.setChecked(False)
    ...
    # ----------------------------------------------------------------
    # CLINICAL TEXT GENERATION (uses "I" for single practitioner, "assessment")
    # ----------------------------------------------------------------
    def _generate_clinical_text(self) -> str:
        ...
        if diagnoses:
            ...
            if self.nature_cb.isChecked() and self.degree_cb.isChecked():
                para1_parts.append("The disorder is both of a nature and degree to warrant detention for assessment.")
            elif self.nature_cb.isChecked():
                para1_parts.append("The disorder is of a nature to warrant detention for assessment.")
            elif self.degree_cb.isChecked():
                para1_parts.append("The disorder is of a degree to warrant detention for assessment.")

            if self.nature_cb.isChecked():
                nature_types = []
                if self.relapsing_cb.isChecked():
                    nature_types.append("relapsing and remitting")
                if self.assessment_resistant_cb.isChecked():
                    nature_types.append("assessment resistant")
        ...
        if necessity_items:
            if len(necessity_items) == 1:
                para2_parts.append(f"Detention for assessment is necessary due to risks to {necessity_items[0]}.")
            elif len(necessity_items) == 2:
                para2_parts.append(f"Detention for assessment is necessary due to risks to {necessity_items[0]} and {necessity_items[1]}.")
            else:
                para2_parts.append(f"Detention for assessment is necessary due to risks to {necessity_items[0]}, {necessity_items[1]} and {necessity_items[2]}.")
        ...
        if self.compliance_cb.isChecked():
            if para3_parts:
                para3_parts.append(f"Compliance with assessment has also been a significant issue and I do not believe {p['subj_l']} would comply if informal.")
            else:
                para3_parts.append(f"Compliance with assessment has been a significant issue and I do not believe {p['subj_l']} would comply if informal.")
    ...
    def _export_docx(self):
        ...
        if not self.prac_section12.isChecked():
            QMessageBox.warning(
                ...
                "Cannot export: The practitioner must be Section 12 approved for Form A4."
            )
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export Form A4",
            f"Form_A4_{datetime.now().strftime('%Y%m%d')}.docx",
            "Word Documents (*.docx)"
        )
        if not file_path:
            return
        ...
        try:
            ...
            template_path = resource_path('templates', 'Form_A4_template.docx')

            if not os.path.exists(template_path):
                QMessageBox.warning(self, "Template Missing", "Form A4 template not found.")
                return
            ...
            # Practitioner details (use field values) - gold bracketed entry box
            prac_parts = []
            ...
            # Patient details (use card content) - gold bracketed entry box
            ...
            # Exam date - gold bracketed entry box
            exam_date = self.prac_exam_date.date().toString("dd MMMM yyyy")
            set_entry_box(paragraphs[8], exam_date)

            # Para 9-10: Wrap acquaintance/section 12 in gold brackets
            # Para 9: "*I had previous acquaintance..." - gold bracket at start
            p9 = paragraphs[9]
            p9_text = ''.join(run.text for run in p9.runs)
            for run in p9.runs:
                run.text = ""
            while len(p9.runs) > 1:
                p9._element.remove(p9.runs[-1]._element)
            if p9.runs:
                p9.runs[0].text = '['
                p9.runs[0].font.bold = True
                p9.runs[0].font.color.rgb = BRACKET_COLOR
                rPr9 = p9.runs[0]._element.get_or_add_rPr()
                shd9 = OxmlElement('w:shd')
                shd9.set(qn('w:val'), 'clear')
                shd9.set(qn('w:color'), 'auto')
                shd9.set(qn('w:fill'), 'FFFED5')
                rPr9.append(shd9)
            p9_content = p9.add_run(p9_text)
            p9_content.font.name = 'Arial'
            p9_content.font.size = Pt(12)
            rPr9c = p9_content._element.get_or_add_rPr()
            shd9c = OxmlElement('w:shd')
            shd9c.set(qn('w:val'), 'clear')
            shd9c.set(qn('w:color'), 'auto')
            shd9c.set(qn('w:fill'), 'FFFED5')
            rPr9c.append(shd9c)
            ...
            if p10.runs:
                p10.runs[0].text = p10_text
                p10.runs[0].font.name = 'Arial'
                p10.runs[0].font.size = Pt(12)
                ...
                rPr10.append(shd10)
            p10_close = p10.add_run(']')
            p10_close.font.name = 'Arial'
            p10_close.font.size = Pt(12)
            p10_close.font.bold = True
            p10_close.font.color.rgb = BRACKET_COLOR
            rPr10c = p10_close._element.get_or_add_rPr()
            ...
            # Checkboxes - strikethrough if not applicable
            ...
            # Para 16: (i) [in the interests of the patient's own health
            p16 = paragraphs[16]
            ...
            if "(i)" in p16_text:
                prefix16 = p16_text[:p16_text.index("(i)") + 3]
                rest16 = p16_text[p16_text.index("(i)") + 3:].lstrip()
            else:
                prefix16 = ""
                rest16 = p16_text
            if p16.runs:
                p16.runs[0].text = prefix16 + ' '
            ...
            rPr16o = p16_open._element.get_or_add_rPr()
            shd16o = OxmlElement('w:shd')
            shd16o.set(qn('w:val'), 'clear')
            shd16o.set(qn('w:color'), 'auto')
            shd16o.set(qn('w:fill'), 'FFFED5')
            rPr16o.append(shd16o)
            p16_content = p16.add_run(rest16)
            ...
            # Para 17: (ii) in the interests of the patient's own safety (cream highlight only)
            p17 = paragraphs[17]
            ...
            if "(ii)" in p17_text:
                prefix17 = p17_text[:p17_text.index("(ii)") + 4]
                rest17 = p17_text[p17_text.index("(ii)") + 4:].lstrip()
            else:
                prefix17 = ""
                rest17 = p17_text
            if p17.runs:
                p17.runs[0].text = prefix17 + ' '
                p17.runs[0].font.name = 'Arial'
                p17.runs[0].font.size = Pt(12)
            p17_content = p17.add_run(rest17)
            ...
            # Para 18: (iii) with a view to the protection of other persons.] (closing bracket)
            p18 = paragraphs[18]
            ...
            if "(iii)" in p18_text:
                prefix18 = p18_text[:p18_text.index("(iii)") + 5]
                rest18 = p18_text[p18_text.index("(iii)") + 5:].lstrip()
            else:
                prefix18 = ""
                rest18 = p18_text
            if p18.runs:
                p18.runs[0].text = prefix18 + ' '
                p18.runs[0].font.name = 'Arial'
                p18.runs[0].font.size = Pt(12)
            p18_content = p18.add_run(rest18)
            ...
            rPr18cl = p18_close._element.get_or_add_rPr()
            shd18cl = OxmlElement('w:shd')
            shd18cl.set(qn('w:val'), 'clear')
            shd18cl.set(qn('w:color'), 'auto')
            shd18cl.set(qn('w:fill'), 'FFFED5')
            rPr18cl.append(shd18cl)
            ...
            # Para 26: "[If you need to continue on a separate sheet please indicate here [ ] and attach that sheet to this form]"
            para26 = paragraphs[26]
            for run in para26.runs:
                run.text = ""
            while len(para26.runs) > 1:
                para26._element.remove(para26.runs[-1]._element)
            if para26.runs:
                para26.runs[0].text = '['
                para26.runs[0].font.name = 'Arial'
                para26.runs[0].font.size = Pt(12)
            text1 = para26.add_run('If you need to continue on a separate sheet please indicate here ')
            text1.font.name = 'Arial'
            text1.font.size = Pt(12)
            cb_open = para26.add_run('[')
            ...
            text2 = para26.add_run(' and attach that sheet to this form]')
            ...
            para28 = paragraphs[28]
            for run in para28.runs:
                run.text = ""
            while len(para28.runs) > 1:
                para28._element.remove(para28.runs[-1]._element)
            if para28.runs:
                para28.runs[0].text = 'Signed'
                para28.runs[0].font.name = 'Arial'
                para28.runs[0].font.size = Pt(12)
            else:
                r = para28.add_run('Signed')
                r.font.name = 'Arial'
                r.font.size = Pt(12)
            sig_open = para28.add_run('[')
            ...
            date_label = para28.add_run('   Date')
            date_label.font.name = 'Arial'
            date_label.font.size = Pt(12)
            date_open = para28.add_run('[')
            ...
            date_content = para28.add_run(sig_date)
            ...
            doc.save(file_path)
            QMessageBox.information(self, "Export Complete", f"Form A4 exported to:\n{file_path}")
    ...
    def get_state(self) -> dict:
        ...
        return {
            ...
            "assessment_resistant": self.assessment_resistant_cb.isChecked(),
    ...
    def load_state(self, state: dict):
        ...
        self.assessment_resistant_cb.setChecked(state.get("assessment_resistant", False))
    ...
    def _fill_patient_details(self, patient_info: dict):
        ...
        if patient_info.get("name") and not self.patient_name.text().strip():
            ...
            print(f"[A4Form] Set patient name: {patient_info['name']}")
        ...
        if patient_info.get("gender"):
            ...
            if not self.gender_male.isChecked() and not self.gender_female.isChecked() and not self.gender_other.isChecked():
                if gender == "male":
                    ...
                    self.gender_female.setChecked(True)
                print(f"[A4Form] Set gender: {gender}")
        ...
        if patient_info.get("ethnicity") and hasattr(self, 'ethnicity_combo'):
            ...
            if current in ("Ethnicity", "Not specified", "Select..."):
                ...
                if idx >= 0:
                    self.ethnicity_combo.setCurrentIndex(idx)
                    print(f"[A4Form] Set ethnicity: {patient_info['ethnicity']}")
