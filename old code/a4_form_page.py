...
class A4FormPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        title = QLabel("Form A4 — Section 2 Medical Recommendation for Assessment")
        ...
        self._create_section_3_practitioner()
        self._create_section_4_clinical_reasons_with_controls()
        self._create_section_5_appropriate_assessment()
        self._create_section_6_signature()
    ...
    # ----------------------------------------------------------------
    # SECTION 3: Medical Practitioner (Single) - Compact Layout
    # ----------------------------------------------------------------
    def _create_section_3_practitioner(self):
        frame = self._create_section_frame(3, "Medical Practitioner")
        layout = frame.layout()

        # Row 1: Name, Address, Email
        row1 = QHBoxLayout()
        row1.setSpacing(12)

        self.prac_name = self._create_line_edit("Full name")
        row1.addWidget(self.prac_name, 1)

        self.prac_address = self._create_line_edit("Address")
        row1.addWidget(self.prac_address, 2)

        self.prac_email = self._create_line_edit("Email")
        row1.addWidget(self.prac_email, 1)
        ...
        row2.addWidget(self.prac_exam_date)

        row2.addSpacing(20)

        self.prac_acquaintance = QCheckBox("Previous acquaintance")
        self.prac_acquaintance.setStyleSheet("font-size: 13px; color: #374151;")
        row2.addWidget(self.prac_acquaintance)

        self.prac_section12 = QCheckBox("Section 12 approved")
        self.prac_section12.setStyleSheet("font-size: 13px; color: #374151;")
        row2.addWidget(self.prac_section12)
    ...
    # ----------------------------------------------------------------
    # SECTION 4: Clinical Reasons with Controls Panel (Split Layout)
    # ----------------------------------------------------------------
    def _create_section_4_clinical_reasons_with_controls(self):
        frame = self._create_section_frame(4, "Clinical Reasons")
        layout = frame.layout()
        ...
        self.assessment_resistant_cb.setStyleSheet("font-size: 12px; color: #6b7280;")
        self.assessment_resistant_cb.toggled.connect(self._update_clinical_text)
    ...
    # ----------------------------------------------------------------
    def _create_section_5_appropriate_assessment(self):
        frame = self._create_section_frame(5, "Appropriate Assessment")
        layout = frame.layout()

        info = QLabel("Specify the hospital(s) where appropriate assessment is available:")
        info.setWordWrap(True)
        info.setStyleSheet("font-size: 12px; color: #6b7280;")
        layout.addWidget(info)

        self.hospital_assessment = self._create_line_edit("Hospital name(s)")
        layout.addWidget(self.hospital_assessment)

        self.form_layout.addWidget(frame)

    # ----------------------------------------------------------------
    # SECTION 6: Signature (Single practitioner)
    # ----------------------------------------------------------------
    def _create_section_6_signature(self):
        frame = self._create_section_frame(6, "Signature")
        layout = frame.layout()
        ...
        info = QLabel("NOTE: The practitioner completing this form must be approved under Section 12 of the Act.")
        info.setWordWrap(True)
    ...
    def _clear_form(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.hospital_assessment.clear()
    ...
    def _export_docx(self):
        ...
        try:
            ...
            template_path = os.path.join(os.path.dirname(__file__), 'templates', 'Form_A4_template.docx')
            ...
            def highlight_yellow(para):
                for run in para.runs:
                    ...
                    shd.set(qn('w:fill'), 'FFFFCC')  # Softer pale yellow
            ...
            # Data goes into WHITESPACE paragraphs below the instruction paragraphs
            # Para 3: Practitioner data (below para 2 which has [PRINT...] instruction)
            # Para 5: Patient data (below para 4 which has [PRINT...] instruction)
            # Para 8: Exam date (below para 7 which has [date] instruction)
            # Para 24: Clinical text (below para 23 which has [Your reasons...] instruction)
            # Para 29: Hospital data (below para 28 text)

            # Practitioner details - goes in para 3 (whitespace below instruction)
            prac_text = self.prac_name.text()
            if self.prac_address.text():
                prac_text += ", " + self.prac_address.text()
            if self.prac_email.text():
                prac_text += ", Email: " + self.prac_email.text()
            if prac_text.strip():
                set_para_text(paragraphs[3], prac_text)
                highlight_yellow(paragraphs[3])

            # Patient details - goes in para 5 (whitespace below instruction)
            ...
            if patient_text.strip():
                set_para_text(paragraphs[5], patient_text)
                highlight_yellow(paragraphs[5])

            # Exam date - goes in para 8 (whitespace below instruction)
            exam_date = self.prac_exam_date.date().toString("dd MMMM yyyy")
            set_para_text(paragraphs[8], exam_date)
            highlight_yellow(paragraphs[8])

            # Checkboxes - strikethrough if not applicable
            # Para 9: "*I had previous acquaintance..."
            # Para 10: "*I am approved under section 12..."
            ...
            # Detention reasons - strikethrough unselected
            ...
            # Clinical reasons - goes in para 24 (whitespace below instruction)
            clinical_text = self._generate_clinical_text()
            if clinical_text:
                set_para_text(paragraphs[24], clinical_text)
                highlight_yellow(paragraphs[24])

            # Hospital for assessment - goes in para 29 (whitespace below instruction)
            # Auto-fill from practitioner address if not specified
            hospital_text = self.hospital_assessment.text().strip()
            if not hospital_text and self.prac_address.text().strip():
                hospital_text = self.prac_address.text().strip()
            if hospital_text:
                set_para_text(paragraphs[29], hospital_text)
                highlight_yellow(paragraphs[29])

            # Signature date - in para 34
            sig_date = self.sig_date.date().toString("dd MMMM yyyy")
            for run in paragraphs[34].runs:
                run.text = ""
            run_new = paragraphs[34].add_run(f"Signed                                                              Date {sig_date}")
            run_new.font.name = 'Arial'
            run_new.font.size = Pt(12)
    ...
    def get_state(self) -> dict:
        ...
        return {
            ...
            "hospital_assessment": self.hospital_assessment.text(),
    ...
    def load_state(self, state: dict):
        ...
        self.hospital_assessment.setText(state.get("hospital_assessment", ""))
