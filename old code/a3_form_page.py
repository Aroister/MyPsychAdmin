...
class A3FormPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._create_section_6_signatures()
    ...
    # ----------------------------------------------------------------
    def _create_section_6_signatures(self):
        frame = self._create_section_frame(6, "Signatures")
        layout = frame.layout()
        ...
        info = QLabel("NOTE: At least one practitioner must be approved under Section 12 of the Act.")
        info.setWordWrap(True)
    ...
    def _export_docx(self):
        ...
        try:
            ...
            template_path = os.path.join(os.path.dirname(__file__), 'templates', 'Form_A3_template.docx')
            ...
            # Para 7: Prac1 data (below para 6 instruction)
            # Para 9: Prac1 date (below para 8 instruction)
            # Para 14: Prac2 data (below para 13 instruction)
            # Para 16: Prac2 date (below para 15 instruction)
            ...
            # First practitioner - goes in para 7
            ...
            if prac1_text.strip():
                set_para_text(paragraphs[7], prac1_text)
                highlight_yellow(paragraphs[7])

            # Prac1 exam date - goes in para 9
            prac1_date = self.prac1_exam_date.date().toString("dd MMMM yyyy")
            set_para_text(paragraphs[9], prac1_date)
            highlight_yellow(paragraphs[9])

            # Prac1 checkboxes (para 10, 11)
            ...
            # Second practitioner - goes in para 14
            ...
            if prac2_text.strip():
                set_para_text(paragraphs[14], prac2_text)
                highlight_yellow(paragraphs[14])

            # Prac2 exam date - goes in para 16
            prac2_date = self.prac2_exam_date.date().toString("dd MMMM yyyy")
            set_para_text(paragraphs[16], prac2_date)
            highlight_yellow(paragraphs[16])

            # Prac2 checkboxes (para 17, 18)
            ...
            for run in paragraphs[34].runs:
                run.text = ""
            paragraphs[34].add_run(f"Signed                                                                    Date {sig1_date}")
            for run in paragraphs[35].runs:
                run.text = ""
            paragraphs[35].add_run(f"Signed                                                                    Date {sig2_date}")
