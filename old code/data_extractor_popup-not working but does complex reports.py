...
class DataExtractorPopup(QWidget):
    ...
    def upload_and_extract(self):
        ...
        # + DATA-LEVEL DEDUPE
        # =================================================
        if dtype == "reports" and auto_report_type == "medical":
            merged = self._latest_panel_data   # 🔒 authoritative replace
        else:
    ...
    def extract_from_rc_report(self, notes):
        ...
        RC_SUBQUESTION_RE = re.compile(
            r"^(is the patient|does the patient|what appropriate|what are the strengths|what is the patient’s understanding|give details of|in the case of|please explain|do you have any recommendations)",
            re.IGNORECASE,
        )

        # =====================================================
        # RC SECTION → CATEGORY MAP (AUTHORITATIVE BY MAJOR NUMBER)
        # =====================================================
        ...
        RC_QUESTION_TEXT_TO_MAJOR = {
                "give details of any index offence": 5,
                "what are the dates of the patient's previous involvement": 6,
                "reasons for any previous admission": 7,
                "what are the circumstances leading up": 8,
                "what appropriate and available medical treatment": 12,
                "give details of any incidents where the patient has harmed": 17,
        }
        ...
        # =====================================================
        # DEBUG — INSPECT RAW DOCX LINES (FIRST 200)
        # =====================================================
        print("\n[RC DEBUG] ===== FIRST 200 RAW LINES =====")
        for i, l in enumerate(lines[:200]):
            print(f"[{i:03d}]", repr(l))
        print("[RC DEBUG] ===== END RAW LINES =====\n")

        category_items = defaultdict(list)
        source_meta = notes[0].get("source_meta", {}) or {}

        # =====================================================
        # NORMALISE DOCX TABLE LINES
        # =====================================================
        def _normalise_docx_line(line: str) -> str:
                return (
                        line
                        .replace("\u00A0", " ")
                        .replace("’", "'")
        ...
        seen_block_hashes_by_section = defaultdict(set)
        seen_line_hashes_by_section = defaultdict(set)

        # =====================================================
        # FLUSH BUFFER (BLOCK-LEVEL DEDUP)
        # =====================================================
        def flush_buffer():
            ...
            block_key = hashlib.md5(block.lower().encode()).hexdigest()
            if block_key in seen_block_hashes_by_section[current_section]:
                buffer.clear()
                return

            seen_block_hashes_by_section[current_section].add(block_key)


            category_items[current_category].append({
                "date": None,
                "text": block,
                "source": {
        ...
        # =====================================================
        # WALK RC SECTIONS (AUTHORITATIVE, MONOTONIC)
        # =====================================================
        for raw_line in lines:
            line = _normalise_docx_line(raw_line).strip()
            if not line:
                continue

            # --------------------------------------------------
            # ✅ RC MAJOR QUESTION HEADER
            # --------------------------------------------------
            num_match = re.match(
                r"^\s*(\d{1,2})\.0\b.*$|^\s*(\d{1,2})\.\s+|^\s*(\d{1,2})\s+",
                line
            )
            if num_match:
                major = int(
                    num_match.group(1)
                    or num_match.group(2)
                    or num_match.group(3)
                )

                if major < 5:
                    continue

                # 🔴 ALWAYS flush previous content
                if current_section is not None:
                    flush_buffer()

                mapped_category = SECTION_CATEGORY_MAP.get(major)

                # 🔒 HARD STOP if this section is unmapped
            ...
            # --------------------------------------------------
            # 🚨 RC SUB-QUESTION HARD STOP (e.g. 5.31)
            # --------------------------------------------------
            ...
            # --------------------------------------------------
            # 🧠 TEXT-ONLY RC HEADER (AUTHORITATIVE)
            # --------------------------------------------------
            lower = line.lower()
            matched_text_header = False

            for phrase, major in RC_QUESTION_TEXT_TO_MAJOR.items():
                ...
                if phrase in lower:
                    ...
                    current_section = major
                    current_category = SECTION_CATEGORY_MAP.get(major)
                    buffer = []

                    matched_text_header = True
                    break

            if matched_text_header:
                continue
            ...
            if SEE_ABOVE_RE.match(line):
                continue
            if RC_SUBQUESTION_RE.match(line):
                continue

            # --------------------------------------------------
            # NORMAL CONTENT — LINE-LEVEL DEDUP
            # --------------------------------------------------
            if current_category:
                line_key = hashlib.md5(line.lower().encode()).hexdigest()

                if line_key in seen_line_hashes_by_section[current_section]:
                    continue

                seen_line_hashes_by_section[current_section].add(line_key)
                buffer.append(line)
        ...
        # =====================================================
        # NORMALISE TO PANEL FORMAT (MATCH MERGE EXPECTATIONS)
        # =====================================================
        normalised = {}
