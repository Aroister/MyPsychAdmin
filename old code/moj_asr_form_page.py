...
class MOJASRFormPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        placeholder.setStyleSheet("font-size: 14px; color: #6b7280; background: white;")
    ...
    def _create_popup_container(self, key: str) -> tuple:
        """Create popup with preview section at top and collapsible input section below."""
        # Main container using QSplitter for reliable preview-at-top positioning
        main_widget = QWidget()
        ...
        # Use QSplitter for consistent layout (preview always at top)
        ...
        preview_inner_layout.setContentsMargins(10, 8, 10, 8)
        preview_inner_layout.setSpacing(6)
        ...
        preview_inner_layout.addLayout(preview_header)

        # Preview text area (dark background)
        ...
        preview_scroll.setStyleSheet("QScrollArea { background: #1e1e1e; border-radius: 6px; }")
        ...
        input_header_label.setStyleSheet("font-size: 13px; font-weight: 600; color: #991b1b; background: transparent; border: none;")
        ...
        # Set initial sizes (preview: 120px, input: rest)
        splitter.setSizes([120, 400])
        splitter.setStretchFactor(0, 0)  # Preview doesn't stretch
        splitter.setStretchFactor(1, 1)  # Input stretches
    ...
    def _create_popup_container_with_imports(self, key: str) -> tuple:
        """Create popup with preview, input, and imported notes in a QSplitter (no overlap)."""
        # Main container
        main_widget = QWidget()
        main_widget.setStyleSheet("background: white;")
        main_layout = QVBoxLayout(main_widget)
        ...
        # Use QSplitter to prevent overlap when resizing
        splitter = QSplitter(Qt.Orientation.Vertical)
        splitter.setHandleWidth(6)
        splitter.setStyleSheet("""
            QSplitter::handle {
        ...
        preview_header = QHBoxLayout()
        preview_title = QLabel("Preview")
        preview_title.setStyleSheet("font-size: 13px; font-weight: 600; color: #991b1b; background: transparent; border: none;")
        preview_header.addWidget(preview_title)
        preview_header.addStretch()
        ...
        send_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        send_btn.setStyleSheet("""
            QPushButton { background: #991b1b; color: white; padding: 6px 14px; border-radius: 5px; font-size: 12px; font-weight: 600; }
            QPushButton:hover { background: #7f1d1d; }
        """)
        preview_header.addWidget(send_btn)
        preview_inner_layout.addLayout(preview_header)
        ...
        preview_label.setWordWrap(True)
        preview_label.setAlignment(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft)
        preview_label.setStyleSheet("QLabel { background: #1e1e1e; color: #eaeaea; padding: 8px; font-size: 12px; border: none; }")
        preview_scroll.setWidget(preview_label)
        preview_inner_layout.addWidget(preview_scroll)
        ...
        import_header.setStyleSheet("""
            QFrame { background: rgba(146, 64, 14, 0.1); border: 1px solid rgba(146, 64, 14, 0.3); border-radius: 6px 6px 0 0; }
        """)
        ...
        import_header_label.setStyleSheet("font-size: 13px; font-weight: 600; color: #92400e; background: transparent; border: none;")
        ...
        import_scroll.setStyleSheet("QScrollArea { background: #fffbeb; border: 1px solid rgba(146,64,14,0.3); border-top: none; border-radius: 0 0 6px 6px; }")
        ...
        # Set initial sizes (preview: 120px, input: stretch, import: 100px)
        splitter.setSizes([120, 400, 100])
        splitter.setStretchFactor(0, 0)  # Preview doesn't stretch
        splitter.setStretchFactor(1, 1)  # Input stretches
        splitter.setStretchFactor(2, 0)  # Import doesn't stretch
    ...
    def _populate_popup_imports(self, key: str, entries: list):
        ...
        for entry in entries:
            ...
            if date:
                ...
                date_lbl.setStyleSheet("font-size: 12px; font-weight: 600; color: #92400e; background: transparent;")
                content_layout.addWidget(date_lbl)

            display_text = text[:120] + "..." if len(text) > 120 else text
            text_lbl = QLabel(display_text)
            text_lbl.setWordWrap(True)
            text_lbl.setStyleSheet("font-size: 12px; color: #374151; background: transparent;")
    ...
    def _add_send_button(self, layout, key: str, generate_func):
        ...
        # Connect the send button in the preview section
        if key in self.popup_send_buttons:
            self.popup_send_buttons[key].clicked.connect(
                lambda checked, k=key: self._send_to_card(k, self.popup_generators[k]())
            )
    ...
    def _update_preview(self, key: str):
        ...
        if key in self.popup_previews and key in self.popup_generators:
            try:
                ...
                self.popup_previews[key].setText(text if text else "(No content)")
    ...
    def _build_popup_patient_details(self):
        ...
        self.popup_patient_name.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
        ...
        self.popup_nhs.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
        layout.addWidget(self.popup_nhs)

        # Hospital
        layout.addWidget(QLabel("Hospital:"))
        self.popup_hospital = QLineEdit()
        self.popup_hospital.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
        layout.addWidget(self.popup_hospital)

        # MHA Section
        layout.addWidget(QLabel("MHA Section:"))
        self.popup_mha_section = QComboBox()
        self.popup_mha_section.addItems(self.MHA_SECTIONS)
        layout.addWidget(self.popup_mha_section)

        # MHCS Reference No.
        layout.addWidget(QLabel("MHCS Reference No.:"))
        self.popup_mhcs_ref = QLineEdit()
        self.popup_mhcs_ref.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
        layout.addWidget(self.popup_mhcs_ref)

        # MHA Section Date
        layout.addWidget(QLabel("MHA Section Date:"))
        ...
        layout.addWidget(QLabel("Other Detention Authorities:"))
        self.popup_other_detention = QLineEdit()
        self.popup_other_detention.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
    ...
    def _build_popup_rc_details(self):
        ...
        layout.addWidget(QLabel("RC Name:"))
        self.popup_rc_name = QLineEdit()
        self.popup_rc_name.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
        if self._my_details.get("full_name"):
            self.popup_rc_name.setText(self._my_details["full_name"])
        layout.addWidget(self.popup_rc_name)

        layout.addWidget(QLabel("RC Email:"))
        self.popup_rc_email = QLineEdit()
        self.popup_rc_email.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
        ...
        self.popup_rc_job_title.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
        ...
        # Telephone No.
        layout.addWidget(QLabel("Telephone No.:"))
        self.popup_rc_phone = QLineEdit()
        self.popup_rc_phone.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
        if self._my_details.get("phone"):
            self.popup_rc_phone.setText(self._my_details["phone"])
        layout.addWidget(self.popup_rc_phone)

        # MHA Office Email
        layout.addWidget(QLabel("Mental Health Act Office Email:"))
        self.popup_mha_office_email = QLineEdit()
        self.popup_mha_office_email.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
    ...
    def _build_popup_mental_disorder(self):
        """Build mental disorder popup with 3 ICD-10 dropdowns."""
        container, layout = self._create_popup_container("mental_disorder")

        layout.addWidget(QLabel("<b>ICD-10 Diagnoses:</b>"))
        layout.addSpacing(8)
        ...
        for i in range(3):
            ...
            lbl.setStyleSheet("font-weight: 500; color: #374151;")
            ...
            combo.setMinimumContentsLength(20)  # Approx characters to show
            combo.addItem("")  # Empty first option

            # Add ICD-10 codes - ICD10_DICT is {diagnosis_name: {severity, diagnosis, icd10}}
            if ICD10_DICT:
                for diagnosis_name, meta in sorted(ICD10_DICT.items()):
                    ...
                    combo.addItem(display_text, diagnosis_name)  # Store diagnosis name as data

            # Enable filtering/completion
            combo.setInsertPolicy(QComboBox.InsertPolicy.NoInsert)
            if ICD10_DICT:
                items = []
                for diagnosis_name, meta in ICD10_DICT.items():
                    icd_code = meta.get("icd10", "") if isinstance(meta, dict) else ""
                    items.append(f"{diagnosis_name} ({icd_code})" if icd_code else diagnosis_name)
        ...
        layout.addWidget(QLabel("<b>Clinical Description:</b>"))
        ...
        self.popup_disorder_desc.setStyleSheet("border: 1px solid #d1d5db; border-radius: 4px; padding: 8px;")
    ...
    def _generate_mental_disorder(self) -> str:
        ...
        for combo in self.popup_dx_combos:
            # Try to get stored data first (diagnosis name), fallback to parsing text
            ...
            if data:
                ...
                if text:
                    # Format is "Diagnosis Name (F20.0)" - strip the ICD code in brackets
                    import re
    ...
    def _build_popup_attitude_behaviour(self):
        ...
        # ============================================
        # PREVIEW SECTION (resizable, at top)
        # ============================================
        if ResizableSection:
            preview_section = ResizableSection()
            preview_section.set_content_height(100)
            preview_section._min_height = 60
            preview_section._max_height = 300
        else:
            preview_section = QWidget()
            preview_section.setFixedHeight(110)
        ...
        if ResizableSection:
            preview_section.set_content(preview_container)
        else:
            fallback_layout = QVBoxLayout(preview_section)
            fallback_layout.setContentsMargins(0, 0, 0, 0)
            fallback_layout.addWidget(preview_container)
        ...
        # ============================================
        # INPUT FIELDS SECTION (collapsible)
        # ============================================
        ...
        for bkey, label, negative_text in behaviour_categories:
            ...
            yes_rb.setStyleSheet("font-size: 11px; color: #059669; background: transparent;")
            no_rb = QRadioButton("No")
            no_rb.setStyleSheet("font-size: 11px; color: #dc2626; background: transparent;")
            ...
            details_input.setStyleSheet("font-size: 11px; padding: 4px 8px; border: 1px solid #d1d5db; border-radius: 4px; background: white;")
            details_input.hide()
        ...
        if CollapsibleSection:
            ...
            import_section.set_title_style("""
                QLabel { font-size: 13px; font-weight: 600; color: #92400e; background: transparent; border: none; }
            """)
        else:
            import_section = QFrame()
            import_section.setStyleSheet("QFrame { background: #fffbeb; border: 1px solid #fcd34d; border-radius: 8px; }")

        import_scroll = QScrollArea()
        import_scroll.setWidgetResizable(True)
        import_scroll.setFrameShape(QScrollArea.Shape.NoFrame)
        import_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        import_scroll.setStyleSheet("QScrollArea { background: #fffbeb; border: none; }")
        ...
        self.popup_behaviour_import_placeholder.setStyleSheet("color: #92400e; font-style: italic; font-size: 11px; background: transparent;")
    ...
    def populate_popup_behaviour_imports(self, entries: list):
        ...
        if category_counts:
            ...
            found_lbl.setStyleSheet("font-size: 11px; font-weight: 600; color: #92400e; background: transparent;")
            ...
            for cat in sorted(category_counts.keys()):
                ...
                btn.setStyleSheet(f"QPushButton {{ font-size: 10px; font-weight: 600; color: white; background: {color}; padding: 2px 6px; border-radius: 4px; border: none; }} QPushButton:hover {{ opacity: 0.8; }}")
        ...
        def create_entry_widget(entry, categories=None):
            ...
            # Use snippet for categorized entries, truncated text for uncategorized
            if categories and entry.get("snippet"):
                display_text = entry["snippet"]
            else:
            ...
            border_color = "#fcd34d"
            if categories:
                # Use first category color for border
                border_color = CATEGORY_COLORS.get(categories[0], "#fcd34d")
            entry_frame.setStyleSheet("QFrame { background: white; border: none; border-radius: 4px; }")
            entry_frame.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Maximum)
            entry_frame.setMinimumWidth(0)
            ...
            # Header row with category tags and expand button
            ...
            if categories:
                for cat in categories:
                    tag = QLabel(cat)
                    color = CATEGORY_COLORS.get(cat, "#6b7280")
                    tag.setStyleSheet(f"font-size: 9px; font-weight: 600; color: white; background: {color}; padding: 1px 4px; border-radius: 3px;")
            ...
            # Text (snippet or truncated)
            text_lbl = QLabel(display_text)
            text_lbl.setWordWrap(True)
            text_lbl.setStyleSheet("font-size: 12px; color: #374151; background: transparent;")
            text_lbl.setTextFormat(Qt.TextFormat.PlainText)
            text_lbl.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Maximum)
            text_lbl.setMinimumWidth(0)
            content_layout.addWidget(text_lbl)

            entry_layout_h.addLayout(content_layout, 1)

            # Store state for expand/collapse
            entry_frame._is_expanded = False
            entry_frame._snippet_text = display_text
            entry_frame._full_highlighted = highlight_keywords(full_text, categories or [])
            entry_frame._text_lbl = text_lbl
            entry_frame._expand_btn = expand_btn

            def toggle_expand(checked, frame=entry_frame, lbl=text_lbl, btn=expand_btn):
                if frame._is_expanded:
                    # Collapse
                    lbl.setTextFormat(Qt.TextFormat.PlainText)
                    lbl.setText(frame._snippet_text)
                    btn.setText("+")
                    frame._is_expanded = False
                else:
                    # Expand with highlighted text
                    lbl.setTextFormat(Qt.TextFormat.RichText)
                    lbl.setText(frame._full_highlighted)
                    btn.setText("−")
    ...
    def _apply_behaviour_filter(self, category):
        ...
        if category:
            ...
            filter_label.setStyleSheet(f"font-size: 12px; font-weight: 600; color: {color}; background: transparent;")
        ...
        for entry, cats in filtered:
            ...
            cb = QCheckBox()
            cb.setStyleSheet("QCheckBox { background: transparent; }")
            cb.stateChanged.connect(lambda state: self._update_preview("behaviour"))
            entry_layout_h.addWidget(cb)
            ...
            expand_btn.setFixedSize(20, 20)
            expand_btn.setCursor(Qt.CursorShape.PointingHandCursor)
            expand_btn.setStyleSheet("QPushButton { font-size: 14px; font-weight: 700; color: #6b7280; background: #f3f4f6; border: 1px solid #d1d5db; border-radius: 4px; } QPushButton:hover { background: #e5e7eb; color: #374151; }")
            header_layout.addWidget(expand_btn)
            content_layout.addLayout(header_layout)
            ...
            entry_frame._is_expanded = False
            entry_frame._snippet_text = display_text
            entry_frame._full_highlighted = highlight_keywords(full_text, cats or [])
    ...
    def populate_popup_patient_attitude_imports(self, entries: list):
        ...
        if category_buttons_layout:
            ...
            if category_counts:
                found_lbl = QLabel("Found:")
                found_lbl.setStyleSheet("font-size: 10px; font-weight: 600; color: #92400e; background: transparent;")
        ...
        def create_entry_widget(entry, categories, filter_callback):
            ...
            cb = QCheckBox()
            cb.setStyleSheet("QCheckBox { background: transparent; }")
            cb.stateChanged.connect(lambda state: self._update_preview("patient_attitude"))
            entry_layout_h.addWidget(cb)

            content_layout = QVBoxLayout()
            content_layout.setSpacing(2)

            # Header row with clickable category tags and expand button
            header_layout = QHBoxLayout()
            header_layout.setSpacing(4)
            ...
            def toggle_expand():
                if entry_frame._is_expanded:
                    # Collapse
                    text_lbl.setTextFormat(Qt.TextFormat.PlainText)
                    text_lbl.setText(entry_frame._snippet_text)
                    expand_btn.setText("+")
                    entry_frame._is_expanded = False
                else:
                    # Expand with highlighted text
                    text_lbl.setTextFormat(Qt.TextFormat.RichText)
                    text_lbl.setText(entry_frame._full_highlighted)
                    expand_btn.setText("−")
                    entry_frame._is_expanded = True
    ...
    def populate_popup_capacity_imports(self, entries: list):
        ...
        def create_entry_widget(entry, categories, filter_callback):
            ...
            # Checkbox for selecting entry
            cb = QCheckBox()
            cb.setStyleSheet("QCheckBox { background: transparent; }")
            cb.stateChanged.connect(lambda state: self._update_preview("capacity"))
            entry_layout_h.addWidget(cb)

            content_layout = QVBoxLayout()
            content_layout.setSpacing(2)

            # Header row with tags and expand button
            header_layout = QHBoxLayout()
            header_layout.setSpacing(4)
    ...
    def populate_popup_risk_addressed_imports(self, entries: list):
        ...
        def create_entry_widget(entry, categories, filter_callback):
            ...
            cb = QCheckBox()
            cb.setStyleSheet("QCheckBox { background: transparent; }")
            cb.stateChanged.connect(lambda state: self._update_preview("risk_addressed"))
            entry_layout_h.addWidget(cb)
    ...
    def populate_popup_abscond_imports(self, entries: list):
        ...
        def create_entry_widget(entry, categories, filter_callback):
            ...
            cb = QCheckBox()
            cb.setStyleSheet("QCheckBox { background: transparent; }")
            cb.stateChanged.connect(lambda state: self._update_preview("abscond"))
            entry_layout_h.addWidget(cb)
    ...
    def populate_popup_mappa_imports(self, entries: list):
        ...
        if category_buttons_layout:
            ...
            if categorized:
                ...
                lbl.setStyleSheet("font-size: 10px; font-weight: 600; color: white; background: #7c3aed; padding: 2px 6px; border-radius: 4px;")
        ...
        for entry in categorized:
            ...
            cb = QCheckBox()
            cb.setStyleSheet("QCheckBox { background: transparent; }")
            cb.stateChanged.connect(lambda state: self._update_preview("mappa"))
            entry_layout_h.addWidget(cb)
            ...
            tag.setStyleSheet("""
                font-size: 9px; font-weight: 600; color: white;
                background: #7c3aed; padding: 1px 4px; border-radius: 3px;
            """)
            header_layout.addWidget(tag)
            header_layout.addStretch()
            ...
            # Store expand/collapse data
            entry_frame._is_expanded = False
            entry_frame._snippet_text = display_text
            entry_frame._full_highlighted = highlight_keywords(full_text, matched_keywords)
            entry_frame._text_lbl = text_lbl
            entry_frame._expand_btn = expand_btn
    ...
    def _build_popup_addressing_issues(self):
        ...
        self.popup_addr_index_label.setStyleSheet("color: #7c3aed; font-weight: 600; font-size: 12px;")
        ...
        self.popup_addr_index_details.setStyleSheet("padding: 6px; border: 1px solid #d1d5db; border-radius: 4px; font-size: 11px;")
        ...
        ot_lbl.setStyleSheet("font-size: 11px;")
        ...
        for i, (key, label) in enumerate(ot_items):
            cb = QCheckBox(label)
            cb.setStyleSheet("font-size: 10px; color: #4b5563;")
        ...
        ot_eng_lbl.setStyleSheet("font-size: 10px; color: #4b5563; min-width: 70px;")
        ...
        ot_eng_row.addWidget(self.popup_addr_ot_slider)

        self.popup_addr_ot_eng_label = QLabel(self.popup_addr_ot_engagement_options[2])
        self.popup_addr_ot_eng_label.setStyleSheet("font-size: 10px; font-weight: 600; color: #7c3aed; min-width: 70px;")
        ...
        psych_eng_lbl.setStyleSheet("font-size: 10px; color: #4b5563; min-width: 70px;")
        ...
        psych_eng_row.addWidget(self.popup_addr_psych_slider)

        self.popup_addr_psych_eng_label = QLabel(self.popup_addr_psych_engagement_options[2])
        self.popup_addr_psych_eng_label.setStyleSheet("font-size: 10px; font-weight: 600; color: #7c3aed; min-width: 70px;")
        ...
        for key, label in risk_factors:
            ...
            # Checkbox
            cb = QCheckBox(label)
            cb.setStyleSheet("font-size: 10px; font-weight: 600; color: #374151; background: transparent;")
            ...
            slider_label.setStyleSheet("font-size: 10px; font-weight: 600; color: #7c3aed; min-width: 100px; background: transparent;")
        ...
        self.risk_concern_title.setStyleSheet("font-size: 11px; color: #991b1b; background: transparent; border: none;")
        ...
        self.risk_concern_slider.setFixedWidth(150)
        self.risk_concern_slider.setStyleSheet(slider_style.replace("#7c3aed", "#dc2626"))
        concern_row.addWidget(self.risk_concern_slider)
        self.risk_concern_label = QLabel(self.concern_options[0])
        self.risk_concern_label.setStyleSheet("font-size: 11px; font-weight: 600; color: #dc2626; min-width: 80px; background: transparent; border: none;")
        ...
        self.risk_concern_details.setStyleSheet("font-size: 10px; padding: 6px; border: 1px solid #d1d5db; border-radius: 4px; background: white;")
        ...
        self.treatment_placeholder.setStyleSheet("color: #6b7280; font-style: italic; font-size: 11px; padding: 8px;")
        ...
        for tx_key, tx_label in treatment_types:
            ...
            tx_cb.setStyleSheet("font-size: 11px; font-weight: 600; color: #374151;")
            ...
            eff_lbl.setStyleSheet("font-size: 10px; color: #4b5563; min-width: 90px;")
            ...
            tx_eff_slider.setFixedWidth(120)
            tx_eff_slider.setStyleSheet(slider_style)
            eff_row.addWidget(tx_eff_slider)
            tx_eff_label = QLabel(effectiveness_options[0])
            tx_eff_label.setStyleSheet("font-size: 10px; font-weight: 600; color: #059669; min-width: 70px;")
        ...
        self.relapse_options = ["Not started", "Just started", "Ongoing", "Significant progression", "Almost completed", "Completed"]
        ...
        relapse_row.addWidget(self.popup_addr_relapse_slider)
        self.popup_addr_relapse_label = QLabel(self.relapse_options[0])
        self.popup_addr_relapse_label.setStyleSheet("font-size: 11px; font-weight: 600; color: #7c3aed; min-width: 120px;")
    ...
    def _update_treatment_risk_radios(self):
        ...
        for key, label in selected_risks:
            ...
            radio.setStyleSheet("""
                QRadioButton { font-size: 10px; font-weight: 600; color: #374151; padding: 4px 8px; }
    ...
    def _build_popup_patient_attitude(self):
        ...
        header_treatment.setStyleSheet("font-size: 11px; font-weight: 600; color: #374151;")
        header_understanding = QLabel("Understanding")
        header_understanding.setStyleSheet("font-size: 11px; font-weight: 600; color: #374151;")
        header_compliance = QLabel("Compliance")
        header_compliance.setStyleSheet("font-size: 11px; font-weight: 600; color: #374151;")
        ...
        insight_row.addWidget(QLabel("Insight into offending:"))
        ...
        self.popup_att_offending_insight_label.setStyleSheet("color: #991b1b; font-weight: 600; min-width: 60px;")
        ...
        insight_row.addWidget(self.popup_att_offending_insight_slider)
        ...
        resp_row.addWidget(QLabel("Accepts responsibility:"))
        ...
        self.popup_att_responsibility_label.setStyleSheet("color: #991b1b; font-weight: 600; min-width: 60px;")
        ...
        resp_row.addWidget(self.popup_att_responsibility_slider)
        ...
        empathy_row.addWidget(QLabel("Victim empathy:"))
        ...
        self.popup_att_empathy_label.setStyleSheet("color: #991b1b; font-weight: 600; min-width: 60px;")
        ...
        empathy_row.addWidget(self.popup_att_empathy_slider)
    ...
    def _build_popup_capacity(self):
        ...
        for area_key, config in areas_config.items():
            ...
            if config["actions"] == "soad":
                ...
                mha_lbl.setStyleSheet("font-size: 11px; color: #374151;")
                ...
                soad_lbl.setStyleSheet("font-size: 11px; color: #374151;")
                ...
                for rb in [fin_none_rb, fin_guardianship_rb, fin_appointeeship_rb, fin_informal_rb]:
                    rb.setStyleSheet("font-size: 11px; color: #374151;")
        ...
        self.popup_cap_details.setStyleSheet("""
            QTextEdit { border: 1px solid #d1d5db; border-radius: 4px; padding: 8px; font-size: 12px; }
            QTextEdit:focus { border-color: #991b1b; }
        """)
    ...
    def populate_popup_progress_imports(self, entries: list):
        """Populate the imported data panel in popup 8 (Progress) with categorized entries.

        Categories based on mental state keywords from tribunal section 14.
        Filters to 12-month window from latest entry. Only shows labeled entries with findings.
        Clickable labels filter by category.
        """
        from datetime import datetime, timedelta
        ...
        def create_entry_widget(entry, categories, filter_callback):
            ...
            cb = QCheckBox()
            cb.setStyleSheet("QCheckBox { background: transparent; }")
            cb.stateChanged.connect(lambda state: self._update_preview("progress"))
            entry_layout_h.addWidget(cb)
            ...
            expand_btn.setCursor(Qt.CursorShape.PointingHandCursor)
            expand_btn.setStyleSheet("""
                QPushButton { background: #f3f4f6; border: 1px solid #d1d5db; border-radius: 4px; font-weight: bold; color: #374151; }
            ...
            if date_obj:
                date_lbl = QLabel(format_date_nice(date_obj))
                date_lbl.setStyleSheet("font-size: 12px; font-weight: 600; color: #92400e; background: transparent;")
                content_layout.addWidget(date_lbl)
            ...
            text_lbl.setWordWrap(True)
            text_lbl.setTextFormat(Qt.TextFormat.RichText)
            text_lbl.setText(highlight_keywords(display_text, categories))
            text_lbl.setStyleSheet("font-size: 12px; color: #374151; background: transparent;")
            content_layout.addWidget(text_lbl)
            ...
            def make_toggle(btn, lbl, disp, full, cats):
                def toggle(checked=None):
                    is_expanded[0] = not is_expanded[0]
                    if is_expanded[0]:
                        btn.setText("−")
                        lbl.setText(highlight_keywords(full, cats))
                    else:
                        btn.setText("+")
                        lbl.setText(highlight_keywords(disp, cats))
                return toggle

            expand_btn.clicked.connect(make_toggle(expand_btn, text_lbl, display_text, full_text, categories))
        ...
        for entry, cats in filtered:
            ...
            for cat in cats:
                ...
                tag.setStyleSheet(f"""
                    ...
                    QPushButton:hover {{ background: {color}; opacity: 0.8; }}
                """)
                tag.clicked.connect(lambda checked, c=cat: self._apply_progress_filter(c))
                header_layout.addWidget(tag)
            ...
            text_lbl.setWordWrap(True)
            text_lbl.setTextFormat(Qt.TextFormat.RichText)
            text_lbl.setText(highlight_keywords(display_text, cats))
            text_lbl.setStyleSheet("font-size: 12px; color: #374151; background: transparent;")
            content_layout.addWidget(text_lbl)
            ...
            def make_toggle(btn, lbl, disp, full, c):
                def toggle(checked=None):
                    is_expanded[0] = not is_expanded[0]
                    if is_expanded[0]:
                        btn.setText("−")
                        lbl.setText(highlight_keywords(full, c))
                    else:
                        btn.setText("+")
                        lbl.setText(highlight_keywords(disp, c))
                return toggle

            expand_btn.clicked.connect(make_toggle(expand_btn, text_lbl, display_text, full_text, cats))
    ...
    def _build_popup_managing_risk(self):
        ...
        current_header.setStyleSheet("font-size: 14px; font-weight: 700; color: #991b1b; background: transparent;")
        ...
        for key, label in self.RISK_TYPES:
            ...
            severity_lbl.setStyleSheet("font-size: 11px; color: #991b1b; font-weight: 600; background: transparent;")
        ...
        historical_header.setStyleSheet("font-size: 14px; font-weight: 700; color: #92400e; background: transparent;")
        ...
        for key, label in self.RISK_TYPES:
            ...
            severity_lbl = QLabel("Low")
            severity_lbl.setFixedWidth(60)
            severity_lbl.setStyleSheet("font-size: 11px; color: #92400e; font-weight: 600; background: transparent;")
    ...
    def _build_popup_risk_addressed(self):
        ...
        prevent_row.addWidget(QLabel("Referred to Prevent?"))
        ...
        self.popup_prevent_outcome.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
    ...
    def _build_popup_abscond(self):
        ...
        if import_layout:
            ...
            filter_label.setStyleSheet("font-size: 11px; color: #6b7280; background: transparent;")
            self.popup_abscond_filter = QComboBox()
            self.popup_abscond_filter.addItem("All")
            self.popup_abscond_filter.setStyleSheet("font-size: 11px; padding: 2px;")
        ...
        self.popup_abscond_details.setStyleSheet("border: 1px solid #d1d5db; border-radius: 4px;")
    ...
    def _generate_abscond(self) -> str:
        if self.popup_awol_no.isChecked():
            return "There have been no AWOL incidents in the last 12 months."
        details = self.popup_abscond_details.toPlainText()
        return f"There have been AWOL incidents. {details}" if details else "There have been AWOL incidents."
    ...
    def _build_popup_mappa(self):
        ...
        input_style = "padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;"
        text_style = """
            QTextEdit {
                border: 1px solid #d1d5db;
                border-radius: 4px;
        ...
        # 1. MAPPA Category
        layout.addWidget(QLabel("<b>1. MAPPA Category:</b>"))
        ...
        # 2. MAPPA Level
        layout.addWidget(QLabel("<b>2. Level at which managed:</b>"))
        ...
        # 3. Date of referral
        layout.addWidget(QLabel("<b>3. Date of referral to MAPPA:</b>"))
        ...
        # 4. MAPPA comments
        layout.addWidget(QLabel("<b>4. MAPPA comments (last 12 months):</b>"))
        ...
        # 5. MAPPA Coordinator
        layout.addWidget(QLabel("<b>5. MAPPA Coordinator name and contact:</b>"))
    ...
    def _build_popup_victims(self):
        ...
        layout.addWidget(QLabel("<b>1. Victim Liaison Officer (VLO) name and contact:</b>"))
        ...
        layout.addWidget(QLabel("<b>2. Date of last discussion/contact with VLO:</b>"))
        ...
        layout.addWidget(QLabel("<b>3. Victim-related concerns (last 12 months):</b>"))
    ...
    def _build_popup_leave_report(self):
        ...
        self.popup_leave_escorted.setStyleSheet("font-weight: 600; font-size: 13px; color: #1e40af;")
        self.popup_leave_unescorted.setStyleSheet("font-weight: 600; font-size: 13px; color: #1e40af;")
        ...
        self.popup_suspension_details.setStyleSheet("QTextEdit { background: white; border: 1px solid #d1d5db; border-radius: 4px; padding: 6px; font-size: 11px; }")
    ...
    def populate_popup_leave_report_imports(self, entries: list):
        ...
        if category_buttons_layout:
            ...
            if categorized:
                ...
                lbl.setStyleSheet("font-size: 10px; font-weight: 600; color: white; background: #6366f1; padding: 2px 6px; border-radius: 4px;")
                category_buttons_layout.addWidget(lbl)
        ...
        for entry in categorized:
            ...
            cb = QCheckBox()
            cb.setStyleSheet("QCheckBox { background: transparent; }")
            cb.stateChanged.connect(lambda state: self._update_preview("leave_report"))
            entry_layout_h.addWidget(cb)
            ...
            tag.setStyleSheet("""
                font-size: 9px; font-weight: 600; color: white;
                background: #6366f1; padding: 1px 4px; border-radius: 3px;
            """)
            header_layout.addWidget(tag)
            header_layout.addStretch()
    ...
    def _build_popup_additional_comments(self):
        ...
        legal_header.setStyleSheet("font-size: 13px; color: #166534;")
        ...
        md_label.setStyleSheet("font-size: 13px; font-weight: 600; color: #374151;")
        ...
        cwd_label.setStyleSheet("font-size: 13px; font-weight: 600; color: #374151;")
        ...
        self.popup_nature_cb.setStyleSheet("font-size: 12px; font-weight: 500; color: #374151;")
        ...
        self.popup_degree_cb.setStyleSheet("font-size: 12px; font-weight: 500; color: #374151;")
        ...
        slider_label.setStyleSheet("font-size: 11px; color: #6b7280;")
        ...
        self.popup_degree_level.setStyleSheet("font-size: 11px; color: #374151; font-weight: 500;")
        ...
        self.popup_degree_details.setStyleSheet("border: 1px solid #d1d5db; border-radius: 6px; padding: 6px; background: white;")
        ...
        necessity_label.setStyleSheet("font-size: 13px; font-weight: 600; color: #374151; margin-top: 8px;")
        ...
        self.popup_health_cb.setStyleSheet("font-size: 12px; font-weight: 500; color: #374151;")
        ...
        self.popup_physical_health_details.setStyleSheet("border: 1px solid #d1d5db; border-radius: 4px; padding: 4px; background: white;")
        ...
        self.popup_safety_cb.setStyleSheet("font-size: 12px; font-weight: 500; color: #374151;")
        ...
        self.popup_self_details.setPlaceholderText("Enter details about risk to self...")
        self.popup_self_details.setMaximumHeight(60)
        self.popup_self_details.setStyleSheet("border: 1px solid #d1d5db; border-radius: 4px; padding: 4px; background: white;")
        self.popup_self_details.textChanged.connect(lambda: self._update_preview("additional_comments"))
        self.popup_self_details.hide()
        safety_layout.addWidget(self.popup_self_details)
        ...
        self.popup_others_details.setPlaceholderText("Enter details about risk to others...")
        self.popup_others_details.setMaximumHeight(60)
        self.popup_others_details.setStyleSheet("border: 1px solid #d1d5db; border-radius: 4px; padding: 4px; background: white;")
        self.popup_others_details.textChanged.connect(lambda: self._update_preview("additional_comments"))
        self.popup_others_details.hide()
        safety_layout.addWidget(self.popup_others_details)
        ...
        self.popup_treatment_cb.setStyleSheet("font-size: 13px; font-weight: 600; color: #374151; margin-top: 8px;")
        ...
        self.popup_least_restrictive_cb.setStyleSheet("font-size: 13px; font-weight: 600; color: #374151;")
    ...
    def _on_popup_self_toggled(self, checked):
        """Toggle Self details visibility."""
        self.popup_self_details.setVisible(checked)
        if not checked:
            self.popup_self_details.clear()
        self._update_preview("additional_comments")

    def _on_popup_others_toggled(self, checked):
        """Toggle Others details visibility."""
        self.popup_others_details.setVisible(checked)
        if not checked:
    ...
    def _generate_additional_comments(self) -> str:
        ...
        if hasattr(self, 'popup_nec_yes') and self.popup_nec_yes.isChecked():
            ...
            # Safety - combined into one sentence
            if hasattr(self, 'popup_safety_cb') and self.popup_safety_cb.isChecked():
                ...
                if self_checked and others_checked:
                    ...
                    if self_details:
                        parts.append(f"Detention is necessary for {p['pos_l']} own safety: {self_details}")
                    else:
                    ...
                    if others_details:
                        parts.append(f"Detention is necessary for the protection of others: {others_details}")
                    else:
    ...
    def _build_popup_unfit_to_plead(self):
        ...
        self.popup_unfit_details.setStyleSheet("border: 1px solid #d1d5db; border-radius: 4px;")
    ...
    def _build_popup_signature(self):
        ...
        layout.addWidget(QLabel("RC Signature:"))
        self.popup_signature = QLineEdit()
        self.popup_signature.setText("Signed electronically")
        self.popup_signature.setStyleSheet("padding: 8px; border: 1px solid #d1d5db; border-radius: 4px;")
    ...
    def _on_data_extracted(self, data: dict):
        ...
        # Get raw notes from the data extractor (like tribunal report does)
        raw_notes = []
        ...
        if document_type == "reports":
            ...
            # Populate Section 4 (Attitude & Behaviour) imports with HPC/PAST_PSYCH
            if hpc_items and hasattr(self, 'populate_popup_behaviour_imports'):
                self.populate_popup_behaviour_imports(hpc_items)
                print(f"[MOJ-ASR] REPORT: Section 4 imports populated with {len(hpc_items)} items")
            ...
            QMessageBox.information(self, "Import Complete", f"Report imported. Found {len(hpc_items)} clinical items, {len(forensic_items)} forensic items.")
    ...
    def _build_section_1_patient_details(self):
        ...
        self.mha_section.setStyleSheet("font-size: 12px; padding: 6px;")
    ...
    def _build_section_3_mental_disorder(self):
        ...
        info_lbl.setStyleSheet("font-size: 12px; color: #6b7280; font-style: italic;")
        ...
        self.mental_disorder_text.setStyleSheet("""
            QTextEdit { background: white; border: 1px solid #d1d5db; border-radius: 6px; padding: 12px; font-size: 13px; }
        ...
        self.diagnosis_changed_cb.setStyleSheet("font-size: 11px; color: #166534;")
    ...
    def _update_mental_disorder_text(self):
        """Update mental disorder text based on control selections."""
        self.mental_disorder_text.setPlainText(self._generate_mental_disorder_text())
    ...
    def _build_section_4_attitude_behaviour(self):
        ...
        prompt_lbl = QLabel("Please describe the patient's attitude and behaviour in the last 12 months, and/or the community, including any incidents:")
        prompt_lbl.setWordWrap(True)
        prompt_lbl.setStyleSheet("font-size: 13px; font-weight: 600; color: #374151;")
        ...
        for key, label, negative_text in behaviour_categories:
            ...
            yes_rb.setStyleSheet("font-size: 10px; color: #059669;")
            yes_rb.setFixedWidth(32)
            no_rb = QRadioButton("N")
            no_rb.setStyleSheet("font-size: 10px; color: #dc2626;")
            ...
            details_input = QLineEdit()
            details_input.setPlaceholderText("Details...")
            details_input.setStyleSheet("font-size: 10px; padding: 2px 4px; border: 1px solid #d1d5db; border-radius: 2px;")
        ...
        self.behaviour_import_placeholder.setStyleSheet("color: #64748b; font-style: italic; font-size: 11px;")
        ...
        if self.import_section:
            ...
            import_title.setStyleSheet("font-size: 12px; font-weight: 700; color: #0369a1;")
    ...
    def populate_behaviour_import_data(self, entries: list):
        ...
        for entry in entries:
            ...
            if date:
                date_lbl = QLabel(str(date))
                date_lbl.setStyleSheet("font-size: 18px; font-weight: 600; color: #64748b;")
                content_layout.addWidget(date_lbl)
            ...
            text_lbl = QLabel(display_text)
            text_lbl.setWordWrap(True)
            text_lbl.setStyleSheet("font-size: 12px; color: #374151;")
            content_layout.addWidget(text_lbl)
    ...
    def _update_behaviour_text(self):
        ...
        if parts:
            current = self.attitude_behaviour_text.toPlainText()
            # Preserve imported notes section if present
            import_marker = "\n\n--- Imported Notes ---\n"
            imported_section = ""
            if import_marker in current:
                imported_section = import_marker + current.split(import_marker)[1]

            # Only auto-update if text is empty or was auto-generated
            if not current or current.startswith("In the last 12 months") or current.startswith("There has been"):
                self.attitude_behaviour_text.setPlainText(" ".join(parts) + imported_section)
            elif imported_section:
                # Keep user text but preserve imported section
                base = current.split(import_marker)[0].rstrip()
                self.attitude_behaviour_text.setPlainText(base + imported_section)
    ...
    def _build_section_5_addressing_issues(self):
        ...
        idx_lbl.setStyleSheet("font-size: 12px; font-weight: 600; color: #1f2937;")
        idx_layout.addWidget(idx_lbl)

        # Progress slider row
        idx_progress_row = QHBoxLayout()
        idx_progress_lbl = QLabel("Progress:")
        idx_progress_lbl.setStyleSheet("font-size: 11px; color: #4b5563; min-width: 60px;")
        ...
        self.index_offence_level.setStyleSheet("font-size: 11px; font-weight: 600; color: #1f2937; min-width: 70px;")
        ...
        idx_eff_lbl.setStyleSheet("font-size: 11px; color: #4b5563; min-width: 60px;")
        ...
        self.index_offence_eff_level.setStyleSheet("font-size: 11px; font-weight: 600; color: #1f2937; min-width: 70px;")
        ...
        self.index_offence_details.setStyleSheet("font-size: 11px; padding: 4px 6px;")
        ...
        risk_lbl.setStyleSheet("font-size: 12px; font-weight: 600; color: #1f2937;")
        risk_layout.addWidget(risk_lbl)

        # Progress slider row
        risk_progress_row = QHBoxLayout()
        risk_progress_lbl = QLabel("Progress:")
        risk_progress_lbl.setStyleSheet("font-size: 11px; color: #4b5563; min-width: 60px;")
        ...
        self.risks_work_level.setStyleSheet("font-size: 11px; font-weight: 600; color: #1f2937; min-width: 70px;")
        ...
        risk_eff_lbl.setStyleSheet("font-size: 11px; color: #4b5563; min-width: 60px;")
        ...
        self.risks_work_eff_level.setStyleSheet("font-size: 11px; font-weight: 600; color: #1f2937; min-width: 70px;")
        ...
        self.risks_work_details.setStyleSheet("font-size: 11px; padding: 4px 6px;")
        ...
        self.ot_cb.setStyleSheet("font-size: 12px; font-weight: 600; color: #374151;")
        ...
        for i, item in enumerate(ot_items):
            cb = QCheckBox(item)
            cb.setStyleSheet("font-size: 11px; color: #4b5563;")
        ...
        ot_concerns_lbl.setStyleSheet("font-size: 11px; color: #4b5563; min-width: 60px;")
        ...
        self.ot_concerns_level.setStyleSheet("font-size: 11px; font-weight: 600; color: #1f2937; min-width: 70px;")
        ...
        self.ot_manageable.setStyleSheet("font-size: 11px; color: #059669;")
        self.ot_not_manageable = QRadioButton("Not Manageable")
        self.ot_not_manageable.setStyleSheet("font-size: 11px; color: #dc2626;")
        ...
        self.psych_groups_cb.setStyleSheet("font-size: 12px; font-weight: 600; color: #374151;")
        ...
        psych_concerns_lbl.setStyleSheet("font-size: 11px; color: #4b5563; min-width: 60px;")
        ...
        self.psych_concerns_level.setStyleSheet("font-size: 11px; font-weight: 600; color: #1f2937; min-width: 70px;")
        ...
        self.psych_manageable.setStyleSheet("font-size: 11px; color: #059669;")
        self.psych_not_manageable = QRadioButton("Not Manageable")
        self.psych_not_manageable.setStyleSheet("font-size: 11px; color: #dc2626;")
        ...
        # Risk items with Understanding dropdown and Engagement slider
        RISK_TYPES = [
            ("violence", "Violence to others"),
            ("self_harm", "Self-harm"),
            ("suicide", "Suicide"),
        ...
        for key, label in RISK_TYPES:
            ...
            risk_cb.setStyleSheet("font-size: 11px; font-weight: 600; color: #374151;")
            ...
            und_lbl.setStyleSheet("font-size: 10px; color: #6b7280;")
            controls_layout.addWidget(und_lbl)

            und_combo = QComboBox()
            und_combo.addItems(["Select...", "Good", "Fair", "Poor"])
            und_combo.setStyleSheet("font-size: 10px;")
            ...
            eng_lbl.setStyleSheet("font-size: 10px; color: #6b7280;")
            ...
            eng_level = QLabel("No")
            eng_level.setStyleSheet("font-size: 10px; color: #6b7280; min-width: 60px;")
        ...
        eff_lbl.setStyleSheet("font-size: 12px; font-weight: 600; color: #92400e;")
        ...
        for level in ["Mild", "Moderate", "High", "Very High"]:
            ...
            rb.setStyleSheet("font-size: 11px; color: #78350f;")
        ...
        relapse_lbl.setStyleSheet("font-size: 12px; font-weight: 600; color: #065f46;")
        ...
        self.relapse_level.setStyleSheet("font-size: 11px; font-weight: 500; color: #047857; min-width: 100px;")
        ...
        self.relapse_refused.setStyleSheet("font-size: 11px; color: #dc2626;")
        ...
        self.section5_import_placeholder.setStyleSheet("color: #64748b; font-style: italic; font-size: 11px;")
    ...
    def _update_addressing_text(self):
        ...
        if parts:
            current = self.addressing_issues_text.toPlainText()
            auto_generated = (not current or current.startswith(p['subj']) or current.startswith("The work") or
                              current.startswith("Work on") or current.startswith("Regarding") or
                              current.startswith("Overall") or current.startswith("Relapse") or
                              current.startswith("There have been"))
            if auto_generated:
                self.addressing_issues_text.setPlainText(" ".join(parts))
    ...
    def _build_section_6_patient_attitude(self):
        ...
        # Headers
        header_treatment = QLabel("Treatment")
        header_treatment.setStyleSheet("font-size: 12px; font-weight: 600; color: #374151;")
        header_understanding = QLabel("Understanding")
        header_understanding.setStyleSheet("font-size: 12px; font-weight: 600; color: #374151;")
        header_compliance = QLabel("Compliance")
        header_compliance.setStyleSheet("font-size: 12px; font-weight: 600; color: #374151;")
        ...
        self.section6_subtitle.setStyleSheet("font-size: 12px; color: #6b7280; font-style: italic;")
    ...
    def _update_attitude_text(self):
        ...
        if parts:
            ...
            auto_generated = (not current or current.startswith(p['subj']) or
                              current.startswith("With respect") or current.startswith("Select"))
            if auto_generated:
                self.patient_attitude_text.setPlainText(" ".join(parts))
    ...
    def populate_section6_import_data(self, entries: list):
        ...
        for i, entry in enumerate(entries):
            ...
            if date:
                ...
                date_label.setStyleSheet("font-size: 11px; font-weight: 600; color: #6b7280;")
    ...
    def _build_section_7_capacity(self):
        ...
        capacity_label.setStyleSheet("font-weight: 600; color: #dc2626; font-size: 12px;")
        right_layout.addWidget(capacity_label)

        domain_label = QLabel("Domain:")
        domain_label.setStyleSheet("font-size: 11px; color: #6b7280;")
        ...
        self.capacity_domain_combo.setStyleSheet("font-size: 11px;")
        ...
        self.capacity_has_rb.setStyleSheet("font-size: 11px; color: #374151;")
        self.capacity_lacks_rb.setStyleSheet("font-size: 11px; color: #374151;")
        ...
        action_label.setStyleSheet("font-weight: 600; color: #dc2626; font-size: 12px;")
        ...
        self.soad_yes_rb.setStyleSheet("font-size: 11px; color: #374151;")
        self.soad_no_rb.setStyleSheet("font-size: 11px; color: #374151;")
        ...
        self.fin_imca_cb.setStyleSheet("font-size: 11px; color: #374151;")
    ...
    def _update_capacity_text(self):
        ...
        self.capacity_text.setPlainText(" ".join(parts))
    ...
    def _build_section_8_progress(self):
        ...
        prog_header.setStyleSheet("font-size: 12px; font-weight: 600; color: #5b21b6;")
        ...
        for key, label, options in progress_options:
            ...
            # Label
            lbl = QLabel(label)
            lbl.setStyleSheet("font-size: 11px; font-weight: 600; color: #5b21b6;")
            ...
            val_lbl.setStyleSheet("font-size: 10px; color: #374151; min-width: 160px;")
            ...
            if key == "leave":
                ...
                usage_lbl.setStyleSheet("font-size: 10px; font-weight: 600; color: #7c3aed;")
                ...
                self.leave_usage_label.setStyleSheet("font-size: 10px; color: #374151; min-width: 160px;")
                ...
                concerns_lbl.setStyleSheet("font-size: 10px; font-weight: 600; color: #dc2626;")
                ...
                self.leave_concerns_label.setStyleSheet("font-size: 10px; color: #374151; min-width: 160px;")
        ...
        discharge_lbl.setStyleSheet("font-size: 11px; font-weight: 600; color: #5b21b6;")
        ...
        for rb in [self.discharge_app_yes, self.discharge_app_no]:
            rb.setStyleSheet("font-size: 10px; color: #374151;")
    ...
    def _update_progress_text(self):
        ...
        self.progress_text.setPlainText(" ".join(parts))

    # ================================================================
    # SECTION 9: Managing Risk - Risk Factors
    # ================================================================
    RISK_TYPES = [
        ("violence", "Violence to others"),
        ("self_harm", "Self-harm"),
    ...
    def _build_section_9_managing_risk(self):
        ...
        current_header.setStyleSheet("font-size: 12px; font-weight: 700; color: #991b1b; background: #fee2e2; padding: 4px 8px; border-radius: 4px;")
        ...
        hist_header.setStyleSheet("font-size: 12px; font-weight: 700; color: #92400e; background: #fef3c7; padding: 4px 8px; border-radius: 4px;")
    ...
    def _create_risk_row(self, key, label, risk_type):
        ...
        slider_lbl.setStyleSheet("font-size: 10px; color: #6b7280;")
        ...
        level_lbl.setStyleSheet("font-size: 10px; color: #374151; font-weight: 500; min-width: 50px;")
    ...
    def _update_risk_text(self):
        ...
        self.risk_factors_text.setPlainText(" ".join(parts))
    ...
    def _build_section_11_abscond(self):
        ...
        awol_header.setStyleSheet("font-size: 11px; font-weight: 600; color: #991b1b;")
        ...
        self.awol_yes_btn = QPushButton("Yes - AWOL incidents")
        self.awol_no_btn = QPushButton("No AWOL incidents")

        for btn in [self.awol_yes_btn, self.awol_no_btn]:
            btn.setCheckable(True)
            btn.setStyleSheet("""
                QPushButton { background: #fee2e2; color: #991b1b; border: 1px solid #fecaca;
                              border-radius: 6px; padding: 6px 10px; font-size: 10px; font-weight: 500; }
        ...
        details_lbl.setStyleSheet("font-size: 10px; font-weight: 600; color: #991b1b;")
        ...
        for key, label in awol_options:
            cb = QCheckBox(label)
            cb.setStyleSheet("font-size: 10px; color: #374151;")
        ...
        self.awol_details_input.setStyleSheet("font-size: 10px; padding: 4px;")
    ...
    def _update_abscond_text(self):
        ...
        if self.awol_no_btn.isChecked():
            self.abscond_text.setPlainText(
            ...
            if parts:
                self.abscond_text.setPlainText(" ".join(parts))
            else:
                self.abscond_text.setPlainText("The patient has had AWOL incidents (details to be provided).")
    ...
    def _build_section_12_mappa(self):
        ...
        cat_lbl.setStyleSheet("font-size: 13px; font-weight: 600; color: #ea580c;")
        ...
        for i, rb in enumerate([self.mappa_cat_na, self.mappa_cat_1, self.mappa_cat_2, self.mappa_cat_3]):
            rb.setStyleSheet("font-size: 12px; color: #374151;")
        ...
        level_lbl.setStyleSheet("font-size: 13px; font-weight: 600; color: #ea580c;")
        ...
        ref_lbl.setStyleSheet("font-size: 13px; font-weight: 600; color: #ea580c;")
        right_layout.addWidget(ref_lbl)

        self.mappa_referral_date = QLineEdit()
        self.mappa_referral_date.setPlaceholderText("DD/MM/YYYY")
        self.mappa_referral_date.setStyleSheet("font-size: 12px; padding: 6px; border: 1px solid #d1d5db; border-radius: 4px;")
        ...
        coord_lbl.setStyleSheet("font-size: 13px; font-weight: 600; color: #374151;")
    ...
    def _build_section_13_victims(self):
        ...
        vlo_lbl.setStyleSheet("font-size: 13px; font-weight: 600; color: #374151;")
    ...
    def _build_section_14_community_leave(self):
        ...
        use_lbl.setStyleSheet("font-size: 14px; font-weight: 600; color: #0891b2;")
        layout.addWidget(use_lbl)

        prompt_lbl = QLabel("Community leave taken (please mark X in box):")
        prompt_lbl.setStyleSheet("font-size: 13px; color: #374151;")
        layout.addWidget(prompt_lbl)
    ...
    def _build_section_14b_leave_details(self):
        ...
        leave_taken_lbl.setStyleSheet("font-size: 12px; font-weight: 600; color: #0891b2;")
        ...
        types_lbl.setStyleSheet("font-size: 11px; font-weight: 600; color: #0891b2;")
        ...
        for type_key, type_label in leave_types:
            ...
            cb.setStyleSheet("font-size: 11px; color: #374151; font-weight: 500;")
            ...
            freq_lbl.setStyleSheet("font-size: 10px; color: #6b7280;")
            ...
            freq_val.setStyleSheet("font-size: 10px; color: #374151; min-width: 55px;")
            ...
            dur_lbl.setStyleSheet("font-size: 10px; color: #6b7280;")
            ...
            dur_val.setStyleSheet("font-size: 10px; color: #374151; min-width: 55px;")
        ...
        attend_lbl.setStyleSheet("font-size: 11px; font-weight: 600; color: #0891b2; margin-top: 4px;")
        ...
        susp_lbl.setStyleSheet("font-size: 11px; font-weight: 600; color: #0891b2; margin-top: 4px;")
        ...
        self.leave_det_susp_details.setStyleSheet("font-size: 11px; padding: 4px; border: 1px solid #d1d5db; border-radius: 4px;")
    ...
    def _update_leave_details_text(self):
        ...
        self.leave_details_text.setPlainText(" ".join(parts))
    ...
    def _clear_form(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            for card in self.cards.values():
                card.editor.clear()
            # Clear all text fields in popups
            ...
            # Reset dates in popups
            if hasattr(self, 'popup_dob'):
                self.popup_dob.setDate(QDate.currentDate())
            if hasattr(self, 'popup_sig_date'):
                self.popup_sig_date.setDate(QDate.currentDate())

            # Clear all imported data sections
            import_keys = [
                "attitude_behaviour", "addressing_issues", "patient_attitude",
                "capacity", "progress", "risk_addressed", "abscond", "mappa", "leave_report"
            ]
            for key in import_keys:
                ...
                cat_layout = getattr(self, f"popup_{key}_category_buttons_layout", None)
                if cat_layout:
                ...
                # Clear imported entries list
                setattr(self, f"popup_{key}_imported_entries", [])

            # Clear section 4 behaviour imports (uses different attribute names)
            if hasattr(self, 'popup_behaviour_import_layout'):
                layout = self.popup_behaviour_import_layout
                while layout.count():
            ...
            if hasattr(self, '_behaviour_imported_entries'):
                self._behaviour_imported_entries = []
            if hasattr(self, '_behaviour_category_labels_container'):
                self._behaviour_category_labels_container.deleteLater()
                self._behaviour_category_labels_container = None
            ...
            # Clear any cached categorized data
    ...
    def _export_docx(self):
        ...
        try:
            ...
            template_path = os.path.join(os.path.dirname(__file__), 'templates', 'MOJ_ASR_template.docx')
            ...
            # Standard left indent for content (0.236 inches as per template)
            LEFT_INDENT = Inches(0.236)
                ...
                for i, para in enumerate(doc.paragraphs):
                    if header_text.lower() in para.text.lower():
                        ...
                        if target_idx < len(doc.paragraphs):
                            ...
                            run.font.color.rgb = RGBColor(0, 0, 0)
                            run.font.size = Pt(11)
                            target_para.paragraph_format.left_indent = LEFT_INDENT
                        return
