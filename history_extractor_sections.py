...
def find_clerkings_rio(notes, admission_dates):
    ...
    for adm in admission_dates:
        ...
        for n in medical_notes:
            ...
            # Allow originator-based detection as backup
            origin = (n.get("originator") or "").lower()
            o_hit = any(r in origin for r in ROLE_TRIGGERS_RIO)

            if not t_hit and not o_hit:
                continue
...
def extract_patient_history(notes, episodes=None, pipeline="rio", debug=False):
    ...
    systems = {n.get("source") for n in notes if n.get("source")}

    if DEBUG:
        print("[HIST] SYSTEMS DETECTED:", systems)

    if "carenotes" in systems:
...
def convert_to_panel_format(history_dict):
    ...
    # Coverage metadata — notes extractor categorises everything by design
    out["_coverage"] = {
        "total_paragraphs": total_items,
        "categorised": total_items,
        "uncategorised": 0,
    }
